...
from camels_es.basins import Config
from camels_es.basins.utils import catchment_polygon
...
def coordinates_coarse(
    ...
    if save:
        root = cfg.points.stem if cfg.points else cfg.points_fine.stem.split('_')[0]

        # polygons
        polygon_file = cfg.output_folder / f'{root}_basins_{cfg.coarse_resolution}.geojson'
        polygons_coarse.to_file(polygon_file, driver='GeoJSON')
        logger.info(f'Basins in the coarser grid have been exported to: {polygon_file}')
        
        # points
        point_file = cfg.output_folder / f'{root}_outlets_{cfg.coarse_resolution}.geojson'
        points_coarse.to_file(point_file, driver='GeoJSON')
        logger.info(f'Outlets in the coarser grid have been exported to: {point_file}')
