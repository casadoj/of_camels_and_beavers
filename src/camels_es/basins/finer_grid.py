...
from camels_es.basins.utils import find_pixel, catchment_polygon
...
def coordinates_fine(
    ...
    if save:
        # polygons
        polygon_file = cfg.output_folder / f'{cfg.points.stem}_basins_{cfg.fine_resolution}.geojson'
        polygons_fine.to_file(polygon_file, driver='GeoJSON')
        logger.info(f'Basins in the finer grid have been exported to: {polygon_file}')
        
        # points
        point_file = cfg.output_folder / f'{cfg.points.stem}_outlets_{cfg.fine_resolution}.geojson'
        points_fine.to_file(point_file, driver='GeoJSON')
        logger.info(f'Outlets in the finer grid have been exported to: {point_file}')
